...
export async function getRamadanTenNightsTemplates(): Promise<TemplateOption[]> {
  ...
  if (error || !data || data.length === 0) {
    ...
    return [
      { id: 1, template_type: 'ramadan_tennights' as any, template_key: 'amanah', template_label: 'My Ten Nights - Amanah', description: 'Modern 6-step wizard configured for Last 10 Nights with "Never Miss Laylatul Qadr" hero, reordered schedule options, and 10-night pricing', is_active: true, sort_order: 1 },
    ];
  }
